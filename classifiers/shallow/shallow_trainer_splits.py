...
def make_split_df(
    ...
    valid_df = df.query('subset == "valid"')
    test_df = df.query('subset == "test"')
...
def main(args):
    ...
    train_df, valid_df, test_df = make_split_df(args.input_csv, args.sources_dir, args.fx_workers)
    logger.info(f"{len(train_df)=}")
    logger.info(f"{len(valid_df)=}")
    ...
    for fx_funcs in fx_funcs_list:
        ...
        X_valid, y_valid = make_Xy(valid_df, fx_funcs)
        X_test, y_test = make_Xy(test_df, fx_funcs)

        # combine train & valid, but use only valid for testing
        ...
        predef_split = PredefinedSplit([-1] * len(X_train) + [0] * len(X_valid))
        ...
        cur["valid_labels"] = y_valid
        cur["valid_preds"] = clf.predict_proba(X_valid)[:, 1]
